...
memtier_benchmark --server="${NODES[0]%%:*}" --port="${NODES[0]##*:}" --authenticate="$CLUSTER_PASS" --protocol=redis --threads=4 --clients=50 --data-size=1024 --ratio=0:1 --test-time=30 | tee "$RESULT_DIR/memtier-throughput.txt"

echo "⚡ Memtier read..."
memtier_benchmark --server="${NODES[1]%%:*}" --port="${NODES[1]##*:}" --authenticate="$CLUSTER_PASS" --protocol=redis --threads=4 --clients=50 --data-size=1024 --ratio=1:0 --test-time=30 | tee "$RESULT_DIR/memtier-read.txt"

echo "⚡ Memtier mixed (80% GET / 20% SET)..."
memtier_benchmark --server="${NODES[0]%%:*}" --port="${NODES[0]##*:}" --authenticate="$CLUSTER_PASS" --protocol=redis --threads=4 --clients=50 --data-size=512 --ratio=8:2 --test-time=30 | tee "$RESULT_DIR/memtier-mix.txt"
